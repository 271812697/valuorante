...
#define XorStringW(str) xorstr(str)
#define XorString(str) xorstr(str)
...
template <std::size_t Size> constexpr std::size_t _buffer_size() {
...
make_xorstr(Tstr str_lambda, std::index_sequence<StringIndices...>,
...
struct LDR_DATA_TABLE_ENTRY_T {
  ...
  INLINE const LDR_DATA_TABLE_ENTRY_T *load_order_next() const noexcept {
...
struct hash_t {
  ...
  INLINE constexpr static value_type single(value_type value, char c) noexcept {
    return static_cast<hash_t::value_type>(
        (value ^ TOLOWER(c)) * static_cast<unsigned long long>(prime));
  }
};
...
INLINE hash_t::value_type hash(const win::UNICODE_STRING_T &str) noexcept {
...
INLINE const win::IMAGE_EXPORT_DIRECTORY *
...
struct module_enumerator {
  ...
  INLINE void reset() noexcept { value = head->load_order_next(); }
...
public:
  ...
  INLINE size_type size() const noexcept { return _ied->NumberOfNames; }
...
INLINE pair<hash_t::value_type, hash_t::value_type>
...
INLINE void *find_module(detail::hash_t::value_type hash_value,
...
INLINE void *find_function(void *dllbase,
                           detail::hash_t::value_type hash_value) {
    ...
    while (export_index--)
      if (detail::hash(exports.name(export_index)) == hash_value) {
        ...
        if (exports.is_forwarded((const char *)addr)) {
          auto hashes =
              detail::hash_forwarded(reinterpret_cast<const char *>(addr));
...
#define QuickFunc(moudle_name, function)                                       \
  constexpr misc::importer::detail::hash_t::value_type                         \
      hash_##moudle##function = misc::importer::detail::khash(moudle_name);    \
  constexpr misc::importer::detail::hash_t::value_type hash_##function =       \
      misc::importer::detail::khash(#function);                                \
  typedef decltype(function) type_##function;                                  \
  type_##function *function =                                                  \
      (type_##function *)misc::importer::find_function(                        \
          misc::importer::find_module(hash_##moudle##function),                \
          hash_##function)
...
template <misc::importer::detail::hash_t::value_type moudle_hash,
          misc::importer::detail::hash_t::value_type function_hash>
struct _get_function_ptr {
  template <typename T> operator T() {
    return (T)misc::importer::find_function(
        misc::importer::find_module(moudle_hash), function_hash);
  }
};

#define GetPtr(moudle_name, function)                                          \
  _get_function_ptr<misc::importer::detail::khash(moudle_name),                \
                    misc::importer::detail::khash(#function)>()
